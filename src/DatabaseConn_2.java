...
public class DatabaseConn_2 {

	public static int save(Student s){
		int status=0;
		try{
			Connection con=DatabaseConn.getCon();
			PreparedStatement ps=con.prepareStatement("insert into student(name,email,course,fee,paid,due,address,contactno) values(?,?,?,?,?,?,?,?)");
	...
	public static int update(Student s){
		...
		try{
			Connection con=DatabaseConn.getCon();
			PreparedStatement ps=con.prepareStatement("update student set name=?,email=?,course=?,fee=?,paid=?,due=?,contactno=? where rollno=?");
			ps.setString(1,s.getName());
			ps.setString(2,s.getEmail());
			ps.setString(3, s.getCourse());
			ps.setInt(4,s.getFee());
	...
	public static Student getStudentByRollno(int rollno){
		...
		try{
			Connection con=DatabaseConn.getCon();
			PreparedStatement ps=con.prepareStatement("select rollno, name, email, course, fee, paid, due, contactno from student where rollno=?");
			ps.setInt(1,rollno);
			ResultSet rs=ps.executeQuery();
			if(rs.next()){
				s.setRollno(rs.getInt("rollno"));
